usps_get_page <- function(street, unit = NULL, city = NULL, state = NULL, zip = NULL) {
  ...
  form <- list(address1 = street,
               address2 = unit,
               city = city,
               state = state,
               zip = zip)
  
  resp <- httr::POST(my_url,
...
usps_get_result <- function(html, n) {
  ...
  result <- c(html$addressList[[n]]$addressLine1,
...
usps_df_all <- function(df) {
  ...
  nm_addr <- c("street", "unit", "city", "state", "zip")
  
  if (!is.data.frame(df)) {
    stop("Input is not a dataframe")
  } else if (!(nm_addr %in% colnames(df))) {
    stop("Expected column names not found")
  } #else if (!("street" %in% colnames(df) & "unit" %in% colnames(df) &
  #            "city" %in% colnames(df) & "state" %in% colnames(df) &
  #            "zip" %in% colnames(df))) {
  #   stop("Expected column names not found")
  # }
  
  nm_other <- colnames(df) - nm_addr
  
  df_usps <- data.frame(matrix(nrow = 0,
                               ncol = 9))
  colnames(df_usps) <- c("n_source", "n_result", "street", "city", "state", "zip5", "zip4", "county", "DPV")
  ...
  withProgress(message = "Getting addresses", {
    for (i in 1:nrow(df)) {
      if (j > 1) {k <- k + j - 1}
      html <- try(usps_get_page(street = df$street[i],
                                unit = df$unit[i],
                                city = df$city[i],
                                state = df$state[i],
                                zip = df$zip[i]))
